...
void _Underworld_Toolbox_Construct( void* component, Stg_ComponentFactory* cf, void* ptrToContext ) {
	...
	/* Only need to initialise a new context, and copy all relevant registers over IF this is the first toolbox */
		...
		context = UnderworldContext_New( curContext->name, 0, 0, curContext->communicator, curContext->dictionary );
		...
		context = (UnderworldContext*)curContext;
	}
