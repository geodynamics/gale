...
for($i = 0; $i < 3; $i++)
{
	...
	while($resIvar < 129){
		while($procvar < 17){
			...
			#outputpathvar	
			$outputPathvar = "./output/MGOPTS/" . $namevar[$i] . "_I" . $resIvar . "_J" . $resJvar . "_P" . $procvar;
			print "$outputPathvar\n";
			#writing the output paths to a text file so we can look em up later to get the cpu time etc etc.
			$outputsfilevar = "outputPathsMGOPTS.txt";
			...
			close (outputpathsfile);
			 $command = "sudo nohup mpiexec -np $procvar ./Underworld ./InputFiles/$filenamevar --elementResI=$resIvar --elementResJ=$resJvar --outputPath=$outputPathvar --mgLevels=$mg -options_file default-GMG-SOR.opts\n";
